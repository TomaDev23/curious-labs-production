  ...
  return (
    <AnimatePresence>
      {open && (
        ...
        >
          <div className="pointer-events-auto mx-auto max-w-3xl overflow-hidden rounded-2xl border border-white/12 bg-[#070b14]/85 shadow-[0_-20px_70px_rgba(0,0,0,0.55)] backdrop-blur-xl">
            {/* header */}
            <div className="flex items-center justify-between border-b border-white/8 px-4 py-2.5">
              ...
              <button
                ...
                onClick={onClose}
                className="flex h-7 w-7 items-center justify-center rounded-md border border-white/12 bg-white/5 text-white/70 transition-colors hover:border-cyan-200/40 hover:text-white focus:outline-none focus:ring-2 focus:ring-cyan-200/60"
              >
            ...
            <div className="grid gap-3 p-3 sm:grid-cols-[minmax(0,0.95fr)_minmax(0,1.15fr)] sm:gap-4 sm:p-4">
              {/* lunar distance board */}
              <div className="rounded-xl border border-white/8 bg-white/[0.02] p-3.5">
                ...
                <div className="mt-1 font-space text-2xl font-semibold tabular-nums text-white">
                  {distK}k <span className="text-sm font-normal text-slate-400">km</span>
                </div>
                {/* perigee → apogee bar */}
                <div className="relative mt-3 h-1.5 rounded-full bg-gradient-to-r from-rose-400/30 via-slate-500/30 to-cyan-400/30">
                  <span
                ...
                <div className="mt-1.5 flex justify-between font-mono text-[8px] uppercase tracking-[0.1em] text-slate-500">
                ...
                <div className="mt-3 grid grid-cols-3 gap-2 border-t border-white/8 pt-3 text-center">
                  {[
                    { k: 'Age', v: `${data.age}d`, c: 'text-slate-200' },
                    { k: 'Light', v: `${data.illumination}%`, c: 'text-cyan-100' },
                    { k: 'Tide', v: data.tide, c: 'text-teal-200' }
                  ].map((s) => (
                    <div key={s.k}>
                      <div className="font-mono text-[8px] uppercase tracking-[0.12em] text-slate-500">{s.k}</div>
                      <div className={`mt-0.5 font-mono text-xs tabular-nums ${s.c}`}>{s.v}</div>
                    </div>
                  ))}
                </div>
              </div>

              {/* phase + condition controls */}
              <div>
                <div className="font-mono text-[9px] uppercase tracking-[0.16em] text-slate-500">Phase — drives the moon</div>
                ...
                <div className="mt-3 font-mono text-[9px] uppercase tracking-[0.16em] text-slate-500">Conditions</div>
                  ...
                  {CONDITIONS.map((c) => {
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-lg border px-3 py-1.5 font-space text-[10px] font-semibold transition-colors ${
                          on
